<script lang="ts">
	...
	import InventoryLoadingState from './InventoryLoadingState.svelte';
	...
	<div class="p-7">
		...
		{:else}
			<div class="mb-8 flex items-baseline gap-2">
				<span class="font-display text-5xl leading-none font-black tracking-tighter text-foreground"
					>{counts.total}</span
				>
				<span
	...
	{:else}
		<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
			{#each resourceGroups as group (group.name)}
				...
				{#if route}
					<a href="/resources/{route}" class={cardClasses}>
						{@render cardContent(group, counts, route, boundedPercent)}
					</a>
				{:else}
