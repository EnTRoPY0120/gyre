...
>
	{#if isOpen || !isMobile}
		...
		<div
			class={cn(
				'flex h-20 flex-col justify-center border-b border-sidebar-border px-6',
		...
		>
			<div class="flex items-center justify-between">
				{#if isOpen}
					...
					<a href="/" class="group flex items-center gap-3">
						...
						<div class="flex flex-col">
							<span class="text-xl leading-tight font-bold tracking-tight text-foreground"
								>Gyre</span
							>
							<div class="mt-0.5">
								<span
				...
				{:else}
					<button
						onclick={() => sidebarOpen.toggle()}
						class="rounded-md p-2 text-muted-foreground transition-all hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:scale-95"
					>
		...
		<div
			class={cn(
				'custom-scrollbar flex-1 space-y-2 overflow-y-auto py-6',
				isOpen ? 'px-4' : 'flex flex-col items-center px-2'
			)}
		>
			{#if isOpen}
				<!-- Dashboard -->
				...
				>
					<Icon
						...
						class={cn(
							'transition-transform group-hover:scale-110',
							currentPath.startsWith('/dashboards') && 'animate-pulse'
						)}
					/>
					Custom Views
				</a>
						...
						<Tooltip.Root>
							...
							<Tooltip.Content side="right">
								<p class="text-xs text-white">
									You need additional permissions to create resources.
								</p>
							</Tooltip.Content>
						</Tooltip.Root>
					</Tooltip.Provider>
