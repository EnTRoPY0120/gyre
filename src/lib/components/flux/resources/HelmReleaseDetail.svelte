...
<div class="space-y-6">
	...
	<div class="rounded-lg border border-gray-200 bg-white p-6">
		<h3 class="mb-4 text-lg font-semibold text-gray-900">Chart Configuration</h3>
		<dl class="grid gap-4 sm:grid-cols-2">
			{#if chart?.spec?.chart}
				<div>
					<dt class="text-sm font-medium text-gray-500">Chart Name</dt>
					<dd class="mt-1">
						<span class="inline-flex items-center gap-1.5 rounded-md bg-blue-100 px-2.5 py-1 text-sm font-medium text-blue-800">
			...
			{#if chart?.spec?.version}
				<div>
					<dt class="text-sm font-medium text-gray-500">Chart Version</dt>
					<dd class="mt-1">
						<span class="inline-flex items-center rounded-md bg-purple-100 px-2 py-1 text-xs font-medium text-purple-800">
							{chart.spec.version}
						</span>
					</dd>
				</div>
					...
					<dd class="mt-1 flex items-center gap-2">
						...
						<span class="text-sm text-gray-900">
							...
							{#if chart.spec.sourceRef.namespace}
								<span class="text-gray-500">({chart.spec.sourceRef.namespace})</span>
							{/if}
						</span>
					</dd>
				</div>
	...
	<div class="rounded-lg border border-gray-200 bg-white p-6">
		<h3 class="mb-4 text-lg font-semibold text-gray-900">Release Configuration</h3>
		<dl class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{#if releaseName}
				<div>
					<dt class="text-sm font-medium text-gray-500">Release Name</dt>
					<dd class="mt-1 text-sm font-medium text-gray-900">{releaseName}</dd>
				</div>
			{/if}
	...
	{#if lastAppliedRevision || lastAttemptedRevision || helmChart}
		<div class="rounded-lg border border-gray-200 bg-white p-6">
			<h3 class="mb-4 text-lg font-semibold text-gray-900">Release Status</h3>
			<dl class="grid gap-4 sm:grid-cols-2">
				...
				{#if helmChart}
					<div class="sm:col-span-2">
						<dt class="text-sm font-medium text-gray-500">HelmChart Reference</dt>
						<dd class="mt-1 font-mono text-sm text-gray-900">{helmChart}</dd>
					</div>
				{/if}
			</dl>
		</div>
	...
	{#if valuesFrom && valuesFrom.length > 0}
		<div class="rounded-lg border border-gray-200 bg-white p-6">
			<h3 class="mb-4 text-lg font-semibold text-gray-900">Values From</h3>
			<div class="flex flex-wrap gap-2">
				{#each valuesFrom as source}
					<span class="inline-flex items-center gap-1.5 rounded-md bg-yellow-100 px-2.5 py-1 text-xs font-medium text-yellow-800">
						<span class="text-yellow-600">{source.kind}:</span>
						{source.name}
					</span>
				{/each}
			</div>
	...
	{#if values && Object.keys(values).length > 0}
		<div class="rounded-lg border border-gray-200 bg-white p-6">
			<h3 class="mb-4 text-lg font-semibold text-gray-900">Inline Values</h3>
