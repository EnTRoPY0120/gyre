...
<div class="space-y-6">
	...
	<div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
		<h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Source Configuration</h3>
		<dl class="grid gap-4 sm:grid-cols-2">
			{#if sourceRef}
				<div>
					<dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Source Reference</dt>
					<dd class="mt-1 flex items-center gap-2">
						<span class="inline-flex items-center gap-1.5 rounded-md bg-blue-100 px-2 py-1 text-xs font-medium text-blue-800 dark:bg-blue-900/50 dark:text-blue-300">
			...
			{#if path}
				<div>
					...
					<dd class="mt-1">
						<code class="rounded bg-gray-100 px-2 py-1 text-sm text-gray-800 dark:bg-gray-700 dark:text-gray-200">{path}</code>
					</dd>
				</div>
			{/if}
	...
	<div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
		<h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Reconciliation Settings</h3>
		<div class="flex flex-wrap gap-3">
			<div class="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 dark:border-gray-600">
				<span class="text-sm font-medium text-gray-500 dark:text-gray-400">Prune</span>
				<span class="inline-flex rounded-md px-2 py-0.5 text-xs font-medium {prune ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'}">
			...
			<div class="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 dark:border-gray-600">
				...
				<span class="inline-flex rounded-md px-2 py-0.5 text-xs font-medium {force ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'}">
			...
			<dl class="grid gap-4 sm:grid-cols-2">
				...
				{#if lastHandledReconcileAt}
					<div>
						...
						<dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{formatTimestamp(lastHandledReconcileAt)}</dd>
					</div>
				{/if}
			</dl>
		</div>
		...
		<div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
			...
			<div class="flex flex-wrap gap-2">
				{#each dependsOn as dep}
					<span class="inline-flex items-center gap-1.5 rounded-md bg-purple-100 px-2.5 py-1 text-xs font-medium text-purple-800 dark:bg-purple-900/50 dark:text-purple-300">
						<svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
			...
			<div class="space-y-2">
				{#each healthChecks as check}
					<div class="flex items-center gap-2 rounded-lg bg-gray-50 px-3 py-2 dark:bg-gray-700/50">
						<span class="inline-flex items-center rounded-md bg-gray-200 px-2 py-0.5 text-xs font-medium text-gray-700 dark:bg-gray-600 dark:text-gray-300">
							{check.kind}
						</span>
	...
	{#if postBuild?.substitute && Object.keys(postBuild.substitute).length > 0}
		<div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
			<h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">PostBuild Substitutions</h3>
			<div class="flex flex-wrap gap-2">
				{#each Object.entries(postBuild.substitute) as [key, value]}
					<span class="inline-flex items-center rounded-md bg-orange-100 px-2 py-1 text-xs font-medium text-orange-800 dark:bg-orange-900/50 dark:text-orange-300">
						<span class="font-semibold">{key}</span>=<span class="text-orange-600 dark:text-orange-400">{value}</span>
					</span>
				{/each}
			</div>
		</div>
