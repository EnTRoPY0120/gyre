...
import { runFlaggedMigration, type Db } from './helpers.js';
...
export function initClusterTables(db: Db): void {
	...
	runFlaggedMigration({
		db,
		key: 'migrations.cluster_context_unique',
		successMessage: '[DB] Migration: added unique constraint on cluster_contexts',
		failureMessage: '[DB] Failed to add unique constraint on cluster_contexts:',
		run: (tx) => {
