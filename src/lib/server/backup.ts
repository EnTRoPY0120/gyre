...
const databaseUrl =
  ...
  (isInCluster ? "/data/gyre.db" : "./data/gyre.db");
const backupDir =
    ...
    .map((filename) => {
      ...
      try {
        ...
        if (parts.length === 2) {
          ...
          const timePart = parts[1].replace(/-/g, (c, offset) =>
            offset < 8 ? ":" : ".",
          );
...
export async function restoreFromBuffer(
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[Backup] Failed to checkpoint current DB before restore:",
        e,
      );
    } finally {
      currentDb.close();
    ...
    for (const suffix of ["-wal", "-shm"]) {
      ...
      try {
        ...
        console.error(
          `[Backup] Failed to remove stale artifact ${artifactPath}:`,
          e,
        );
      }
    }
