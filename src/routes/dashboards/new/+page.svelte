<script lang="ts">
	import { dashboardStore, type Dashboard } from '$lib/stores/dashboards.svelte';
	...
	<div class="rounded-xl border border-border bg-card p-6 shadow-sm">
		<form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="space-y-6">
			...
			<div class="space-y-2">
				...
				<Textarea id="description" bind:value={description} placeholder="Describe the purpose of this dashboard" rows={3} />
			</div>

			<div class="flex items-center space-x-2">
				<Checkbox id="shared" bind:checked={isShared} />
				<Label for="shared" class="font-normal cursor-pointer">
                    Share with other users
                    <span class="block text-xs text-muted-foreground">Allow other users to view this dashboard</span>
                </Label>
			</div>
