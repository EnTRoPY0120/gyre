...
export const POST: RequestHandler = async ({ params, locals, getClientAddress }) => {
	...
	try {
		await toggleSuspendResource(resolvedType, namespace, name, false, locals.cluster);

		// Log the audit event
		await logResourceWrite(locals.user, resolvedType, 'resume', name, namespace, locals.cluster, {
